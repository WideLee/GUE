...
public class WeatherCard extends RecyclableCard {
	...
	Weather mWeather = WeatherUtil.getInstance().getWeatherList()
		.get(mIndex);

	TextView cityNameTextView = ((TextView) convertView
	...
	TextView overviewTextView = ((TextView) convertView
		.findViewById(R.id.tv_overview));
	overviewTextView.setText(mWeather.getOverView());

	TextView uvaTextView = ((TextView) convertView
		.findViewById(R.id.tv_uva));
	...
	weatherIconImageView.setBackgroundResource(R.drawable.weather_00
		+ index);

	TextView loadingTextView = (TextView) convertView
		.findViewById(R.id.tv_loading);
	RelativeLayout weatherLayout = (RelativeLayout) convertView
		.findViewById(R.id.rl_weather);
	if (mWeather.isInit()) {
