...
public class CardFactory {
	 ...
	 */
	public static AbstractCard createCard(CardModel model) throws InstantiationException,
		...
		for (int i = 0; i < sourceFields.length; i++) {
			...
			if (destField != null) {
				...
				Log.d("CardFactory",
						String.format(" > Field %s (= %s) -> Field %s (=%s)", curField,
								curField.get(model), destField.getName(), destField.get(newCard)));
			} else {
				/*
