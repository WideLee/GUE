...
public class GetUpEarlyDB extends SQLiteOpenHelper {
    ...
    private static final String EVENT_SQL_CREATE = "create table "
	    + EVENT_TABLE + " ( " + COLUMN_KEY_EID
	    + " integer primary key autoincrement, " + COLUMN_IS_SIGN_IN
	    + " integer, " + COLUMN_IS_DONE + " integer, " + COLUMN_EVENT_TIME
	    + " text, " + COLUMN_EVENT_CONTENT + " text);";
	...
	values.put(COLUMN_EVENT_TIME,
		dateFormat.format(new Date(entity.getTime())));
	...
	Cursor c = db.query(EVENT_TABLE, null, selection, selectionArgs, null,
		null, null);
	if (c.moveToNext()) {
	    event = new Event(c.getInt(c.getColumnIndex(COLUMN_IS_SIGN_IN)),
		    c.getInt(c.getColumnIndex(COLUMN_IS_DONE)), dateFormat
			    .parse(c.getString(c
				    .getColumnIndex(COLUMN_EVENT_TIME)))
			    .getTime(), c.getString(c
     ...
     */
    public ArrayList<Event> getEventByDate(Date date, int choose)
	    throws ParseException {
	...
	String SEARCH_EVENT = "select * from " + EVENT_TABLE
		+ " where  ( datetime(" + COLUMN_EVENT_TIME + ") < \""
		+ dateEnd + "\" and " + "datetime(" + COLUMN_EVENT_TIME
		+ ") > \"" + dateBegin + "\" );";
	...
	while (c.moveToNext()) {
	    if (!is_ava
		    || (is_ava && (c
			    .getInt(c.getColumnIndex(COLUMN_IS_SIGN_IN)) == choose))) {
		Event event = new Event(c.getInt(c
			.getColumnIndex(COLUMN_IS_SIGN_IN)), c.getInt(c
			.getColumnIndex(COLUMN_IS_DONE)), dateFormat.parse(
			c.getString(c.getColumnIndex(COLUMN_EVENT_TIME)))
			.getTime(), c.getString(c
			.getColumnIndex(COLUMN_EVENT_CONTENT)));
		event.setId(c.getLong(c.getColumnIndex(COLUMN_KEY_EID)));
		list.add(event);
	...
	ArrayList<Event> list = new ArrayList<Event>();
	SQLiteDatabase db = getReadableDatabase();
	Cursor c = db.query(EVENT_TABLE, null, null, null, null, null,
		COLUMN_EVENT_TIME);
